...
from core.util import str_to_float
from tabs.schema.graph import Node, StreamType, Handle
...
class Table(QTableWidget):
    ...
    def add_stream(self, handle: Handle):
        ...
        # Log:
        print(f"Fetching handle {handle.symbol}: {handle.cname} {handle.value}")
    ...
    def add_params(self, resource: Resource = Resource()):
        ...
        # Create QTableWidgetItems:
        symb_item = QTableWidgetItem(QIcon("rss/icons/parameter.png"), resource.symbol)
        symb_item.setData(Qt.ItemDataRole.UserRole, "Parameter")

        name_item = QTableWidgetItem(resource.info)
        unit_item = QTableWidgetItem(resource.units)
    ...
    def commit(self):
        ...
        for row in range(self.rowCount()):
            ...
            if row in self._hmap.keys():
                ...
                variable.cname   = self.cell_data(row, 3)
                ...
                if variable.connected and variable.conjugate:
                    ...
                    conjugate.cname   = self.cell_data(row, 3)
            ...
            # Update the node's parameters:
                ...
                resource = Resource()
                resource.symbol = self.cell_data(row, 0)
                resource.info   = self.cell_data(row, 1)
                resource.units  = self.cell_data(row, 2)
                resource.cname  = self.cell_data(row, 3)
                resource.value  = self.cell_data(row, 4)
                resource.lower  = self.cell_data(row, 5)
                resource.upper  = self.cell_data(row, 6)
                resource.sigma  = self.cell_data(row, 7)

                # Add parameter to dictionary:
                parameters[resource] = True

        # Commit dictionary to node:
        self._node().parameters = parameters
